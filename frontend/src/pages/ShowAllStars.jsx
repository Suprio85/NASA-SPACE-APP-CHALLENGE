...
const EXOPLANET_API_URL = 'https://cors-anywhere.herokuapp.com/https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+distinct+top+1000+pl_name,pl_orbper,pl_rade,pl_masse,disc_year,discoverymethod,hostname,st_teff,st_rad,st_mass,disc_facility,pl_eqt+from+ps&format=json';
...
const StarPlanetPage = () => {
    ...
    const showAnimation = (stars) => {
        setShowAnimate(!showAnimate); // Toggle animation state
        setSelectedSolarSystem(stars);
    };

    const handlePlanetCountChange = (event) => {
        setPlanetCount(event.target.value); // Update the state when the slider changes
        setCurrentPage(1);  // Reset page when filter changes
    };
    const resetCount = (event) => {
        setPlanetCount(0); // Update the state when the slider changes
        setCurrentPage(1);  // Reset page when filter changes
    };
    ...
    useEffect(() => {
        const fetchStarsAndPlanets = async () => {
            try {
                ...
                setStars(Object.values(starData));
    ...
    // Automatically adjust the page number if the current page has no stars
    useEffect(() => {
        const totalFilteredStars = filteredStars.length;
        const maxPage = Math.ceil(totalFilteredStars / starsPerPage);

        // If the current page exceeds the available pages, set it to the last page
        if (currentPage > maxPage && maxPage > 0) {
            setCurrentPage(maxPage);
        }
    }, [filteredStars, currentPage]);

    // Filter stars by planet count
    const starsToDisplay = filteredStars.filter(star => {
        return planetCount == 0 || star.planets.length === parseInt(planetCount);
    });
    ...
    const currentStars = starsToDisplay.slice(indexOfFirstStar, indexOfLastStar);
    const totalPages = Math.ceil(starsToDisplay.length / starsPerPage);
    ...
    return (
        <div className="min-h-screen absolute top-0 text-white w-full ">
            ...
            <div className='flex justify-end w-full mb-20'>
                <div className="collapse w-full ml-60 ">
                    <input type="checkbox" />
                    <div className="collapse-title text-xl bg-transparent font-Saira font-bold">Filter</div>
                    <div className="collapse-content">
                        <div className='flex justify-start w-1/2 items-center z-50 pl-0 border-t-2 pt-5 border-slate-600'>
                            <div className='mr-10 font-Saira text-xl text-white'>Planet Count</div>
                            <div className='w-72'>
                                <input type="range" min="1" max="8" value={planetCount} className="range" step="1" onChange={handlePlanetCountChange} />
                            ...
                            <div className='ml-10 font-Saira text-xl text-white hover:cursor-pointer' onClick={resetCount}>Reset</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {showAnimate && <SolarSystem planets={selectedSolarSystem.planets} showAnimation={showAnimation} />}

            {/* Star Card Display */}
            <div className="w-auto mx-60 hover:cursor-pointer">
                {currentStars.map((star, index) => (
                    <div onClick={() => showAnimation(star)} className="hover:ring" key={index}>
                        {console.log(star.planets.length)},
                        {/* Show stars based on planet count or show all stars if planetCount is 0 */}
                        {planetCount === 0 || star.planets.length === parseInt(planetCount) ? (
