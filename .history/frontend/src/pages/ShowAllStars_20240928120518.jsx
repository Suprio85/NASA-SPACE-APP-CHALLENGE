...
const EXOPLANET_API_URL = 'https://cors-anywhere.herokuapp.com/https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+distinct+top+1000+pl_name,pl_orbper,pl_rade,pl_masse,disc_year,discoverymethod,hostname,st_teff,st_rad,st_mass,disc_facility+from+ps&format=json';
...
const StarCard = ({ star }) => {
    ...
    return (
        <div className="bg-gray-800 bg-opacity-35 overflow-hidden h-96 relative rounded-lg p-6 shadow-lg mb-5">
            <div className='absolute flex justify-center items-center top-10 left-10'>
                <div className='rounded-full bg-white w-96 h-96 absolute -z-10'>

                </div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'550px ',width:'550px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'730px ',width:'730px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'910px ',width:'910px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'1100px ',width:'1100px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'1300px ',width:'1300px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'1500px ',width:'1500px'}}></div>
                <div className='rounded-full border-white border-2  absolute -z-10' style={{height:'1700px ',width:'1700px'}}></div>
            </div>
            <div className="flex items-center justify-start">
                <div className="w-24 h-24 bg-yellow-500 rounded-full flex justify-center items-center mb-2">
                    {/* <img src={starImageUrl} alt={starDetails.hostname} className="w-full h-full object-cover rounded-full" /> */}
                </div>
                <h2 className="text-3xl font-bold ml-6">{starDetails.hostname}</h2>
            </div>

            <div className="flex mt-4 ml-56">
                {/* Planet Balls */}
                {planets.map((planet, index) => (
                    <div key={index} className="mx-10 w-12 h-12 bg-blue-500 rounded-full mr-4">{planet.pl_name}</div>
                ))}
            </div>
        </div>
    );
