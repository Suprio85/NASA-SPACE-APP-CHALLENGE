    ...
    useEffect(() => {
        const fetchPlanetImage = async () => {
            try {
                ...
                // Add the term "exoplanet" to increase relevance for the image search
                const response = await fetch(`${NASA_IMAGE_API_URL}?q=${planetName}+exoplanet`);
                const data = await response.json();

                if (data.collection.items.length > 0) {
                    // Check for relevant metadata that matches the planet context
                    ...
                    if (filteredItems.length > 0) {
                        ...
                        setImageUrl('https://via.placeholder.com/150'); // Fallback if no exact match found
                    }
                } else {
