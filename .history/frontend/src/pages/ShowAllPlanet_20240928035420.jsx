...
const ExoplanetCard = ({ planet }) => {
            ...
            try {
                ...
                if (data.collection.items.length > 0) {
                    ...
                    if (filteredItems.length > 0) {
                        ...
                        // If no relevant images, try secondary search (just "exoplanet")
                        fetchGenericExoplanetImage();
                    }
                } else {
                    // If no images found, try secondary search (just "exoplanet")
                    fetchGenericExoplanetImage();
                }
            } catch (err) {
                console.error('Error fetching image:', err);
                fetchGenericExoplanetImage(); // Fallback on error
            }
        };

        const fetchGenericExoplanetImage = async () => {
            try {
                const response = await fetch(`${NASA_IMAGE_API_URL}?q=exoplanet`);
                const data = await response.json();

                if (data.collection.items.length > 0) {
                    setImageUrl(data.collection.items[0].links[0].href); // Use first generic exoplanet image
                } else {
                    setImageUrl('https://via.placeholder.com/150'); // Fallback if no image found
                }
            } catch (err) {
                console.error('Error fetching generic exoplanet image:', err);
                setImageUrl('https://via.placeholder.com/150'); // Fallback on error
            }
        };
    ...
    return (
        <div className="bg-gray-800 overflow-hidden relative rounded-lg p-6 shadow-lg -z-10 mb-5">
            <div className='absolute overflow-hidden -right-20 -top-60 -z-10'>
                <img src={planet7} alt="Planet" className="w-full object-cover rounded-lg mb-4 opacity-50" style={{ height: '800px' }} />
            </div>
            <div className=''>
