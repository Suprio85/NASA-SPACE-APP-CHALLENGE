...
const EXOPLANET_API_URL = 'https://cors-anywhere.herokuapp.com/https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+pl_name,disc_year,pl_masse,pl_rade+from+ps&format=json';
...
const ExoplanetPage = () => {
    ...
    useEffect(() => {
        const fetchExoplanets = async () => {
            try {
                ...
                setExoplanets(data.slice(0, 100)); // Show only the first 100 exoplanets
                setFilteredPlanets(data.slice(0, 100)); // Also apply limit to filtered planets
                setLoading(false);
            } catch (err) {
    ...
    const handleSearch = (e) => {
        ...
        const filtered = exoplanets
            ...
            .slice(0, 100); // Ensure only the first 100 results are displayed
