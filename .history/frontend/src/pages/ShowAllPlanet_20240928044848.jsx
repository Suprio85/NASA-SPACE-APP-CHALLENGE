...
const EXOPLANET_API_URL = 'https://cors-anywhere.herokuapp.com/https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+distinct+top+200+pl_name,disc_year,pl_masse,pl_rade,pl_orbper,discoverymethod,st_teff,st_rad,st_mass,disc_facility+from+ps&format=json';
...
const ExoplanetPage = () => {
    ...
    if (loading) return (<div className='flex flex-col justify-center items-center h-screen w-full absolute top-0'>
        <div className="text-center text-white ">Creating Planets...</div>
        <div><span class="loading loading-infinity loading-lg"></span></div>
    </div>
    )
...
const ExoplanetCard = ({ planet }) => {
    ...
    return (
        <div className="bg-gray-800 overflow-hidden relative rounded-lg p-6 shadow-lg -z-10 mb-5">
            <div className='absolute overflow-hidden -right-20 -top-32 -z-10 opacity-50'>
                <img src={planet7} alt="Planet" className="w-full object-cover rounded-lg mb-4" style={{ height: '500px' }} />
            </div>
            <div className='absolute overflow-hidden -left-20 -bottom-32 -z-10 opacity-70'>
                <img src={planet8} alt="Planet" className="w-full object-cover rounded-lg mb-4" style={{ height: '500px' }} />
            </div>
            <div className='flex items-center '>
                <div className='w-1/2  justify-center'>
                    <h2 className="text-3xl font-bold font-Saira mb-2 z-20">{planet.pl_name}</h2>
                ...
                <div className='w-1/2'>
                    <img src={imageUrl} alt={planet.pl_name} className=" z-20 w-full h-80 object-cover rounded-lg " />
                </div>
...
const SmallBox = ({ label, value }) => {
    return (
        <div className='my-2  h-20 flex justify-center items-center flex-col bg-slate-700 rounded-md'>
            <div className=' h-2/3  w-full flex justify-center items-center text-xl font-Titiliuam'>{value}</div>
            <div className='font-bold h-1/3 w-full flex justify-center items-center font-Titiliuam bg-slate-600 bg-opacity-90 rounded-bl-md rounded-br-md'>{label}</div>
        </div>
    );
};
