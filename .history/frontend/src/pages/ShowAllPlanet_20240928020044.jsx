...
// Correct NASA Exoplanet Archive API URL with CORS Anywhere
const EXOPLANET_API_URL = 'https://cors-anywhere.herokuapp.com/https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+distinct+pl_name,disc_year,pl_masse,pl_rade+from+ps+limit+100&format=json';
...
const ExoplanetPage = () => {
    ...
    // Fetch exoplanets from NASA API (limit to 100 unique results)
    useEffect(() => {
        const fetchExoplanets = async () => {
            try {
                const response = await fetch(EXOPLANET_API_URL, {
                    headers: {
                        'origin': 'https://your-frontend-domain.com', // Use your domain here or localhost
                ...
                setExoplanets(data); // Fetch 100 unique exoplanets
