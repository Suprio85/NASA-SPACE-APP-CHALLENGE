...
const LatestDiscovery = () => {
    ...
    useEffect(() => {
      // Fetch the latest exoplanet data
      const fetchLatestExoplanets = async () => {
        try {
          const response = await axios.get(
            `https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+pl_name,hostname,disc_year,discoverymethod+from+pscomppars+order+by+disc_year+desc+limit+5&format=json`
          );
    ...
    return (
      <div className="bg-slate-950 py-20 px-60 mt-20">
        ...
        <div className="w-full font-Saira text-slate-100 text-xl ">
          Check out the latest discoveries in the field of exoplanets:
        </div>
  
        <div className="mt-6">
          {latestExoplanets.length > 0 ? (
            <ul className="text-slate-100 text-lg space-y-4">
              {latestExoplanets.map((planet, index) => (
                <li key={index}>
                  <strong>{planet.pl_name}</strong> orbiting <em>{planet.hostname}</em>, discovered in {planet.disc_year} via {planet.discoverymethod}.
                </li>
              ))}
            </ul>
          ) : (
