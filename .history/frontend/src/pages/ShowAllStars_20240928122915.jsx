...
const StarCard = ({ star, index }) => {
    ...
    return (
        <div className={`bg-gray-800 bg-opacity-0 overflow-hidden h-96 relative rounded-lg p-6 shadow-lg mb-5 ${isEvenIndex ? 'flex-row' : 'flex-row-reverse'} flex justify-between`}>
            <div className='relative'>
                {/* Star Ball with Circles */}
                <div className={`absolute flex ${isEvenIndex ? 'left-10' : 'right-10'} top-10`}>
                ...
                <div className={`flex ${isEvenIndex ? 'items-end' : 'items-start'} justify-start`}>
                    <h2 className={`text-3xl font-bold ${isEvenIndex ? 'ml-6' : 'mr-6'}`}>{starDetails.hostname}</h2>
                </div>
            </div>

            <div className={`flex mt-4 ${isEvenIndex ? 'ml-52' : 'mr-52'}`}>
                {/* Planet Balls */}
                {planets.map((planet, index) => (
                    <div key={index} className='flex justify-center items-center flex-col font-Saira'>
