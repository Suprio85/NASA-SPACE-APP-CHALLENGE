...
const LatestDiscovery = () => {
  ...
  useEffect(() => {
    const fetchLatestExoplanets = async () => {
      try {
        ...
        const exoplanetResponse = await axios.get(
        ...
        const exoplanets = exoplanetResponse.data;
        
        // For each exoplanet, fetch an image from the NASA Image API
        const exoplanetsWithImages = await Promise.all(
          exoplanets.map(async (planet) => {
            const imageResponse = await axios.get(
              `https://images-api.nasa.gov/search?q=${encodeURIComponent(planet.pl_name)}+exoplanet+illustration&media_type=image`
            );
            
            // Get the first image result if available, else use a placeholder
            const image = imageResponse.data.collection.items.length > 0
              ? imageResponse.data.collection.items[0].links[0].href
              : 'https://via.placeholder.com/300x200?text=No+Image+Available';
            
            return { ...planet, image }; // Add image to the exoplanet data
          })
        );
        
        setLatestExoplanets(exoplanetsWithImages);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching exoplanet data or images", error);
  ...
  return (
    <div className="bg-slate-950 py-20 px-60 mt-20">
        ...
        {latestExoplanets.length > 0 ? (
          latestExoplanets.map((planet, index) => (
            <div key={index} className="bg-slate-800 p-6 rounded-lg shadow-lg text-slate-100">
              <img
                src={planet.image} // Display the fetched image
                alt={planet.pl_name}
...
const HomePage = () => {
    return (
        <div className="login-page overflow-hidden">
            ...
            <div className=''>
                <div className='bg-white w-full h-screen fixed -z-50'></div>
                <Hero />
                <Storytelling />
                <ExoplanetImages />
                <LatestDiscovery />
