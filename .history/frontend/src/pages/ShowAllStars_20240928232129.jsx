...
const planetImages = [planet1, planet2, planet3, planet4, planet5, planet6, planet7, planet8];
...
const StarPlanetPage = () => {
    ...
    if (showAnimate && selectedStar) {
        return <StarAnimation star={selectedStar} />;
    }
        ...
        <div className="min-h-screen absolute top-0 text-white w-full ">
            ...
            <div className="w-auto mx-60">
                {currentStars.map((star, index) => (
                    <StarCard key={index} star={star} index={index} onAnimateClick={handleAnimate} />
                ))}
            </div>
...
const StarAnimation = ({ star }) => {
    return (
        <div className="flex justify-center items-center h-screen bg-black text-white">
            <div className="text-center">
                <h2 className="text-4xl font-bold mb-8">{star.starDetails.hostname} System</h2>
                <div className="flex justify-center items-center">
                    <div className="bg-yellow-500 rounded-full h-32 w-32"></div>
                    <div className="ml-8 text-2xl">
                        {star.planets.map((planet, index) => (
                            <div key={index} className="mb-4">
                                Planet: {planet.pl_name} - Orbital Period: {planet.pl_orbper}
                            </div>
                        ))}
                    </div>
                </div>
...
// StarCard Component
const StarCard = ({ star, index, onAnimateClick }) => {
    const { starDetails, planets } = star;
    const planetImages = [planet1, planet2, planet3, planet4, planet5, planet6, planet7, planet8];

    return (
        <div className={`bg-gray-800 overflow-hidden h-96 relative rounded-lg p-6 shadow-lg mb-5 flex ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
            ...
            <div className={`flex ${index % 2 === 0 ? 'ml-40' : 'mr-48'}`}>
                {planets.map((planet, planetIndex) => (
                    <div key={planetIndex} className='flex justify-center items-center flex-col font-Saira'>
