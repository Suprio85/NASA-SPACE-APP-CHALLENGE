...
const StarPlanetPage = () => {
    ...
    const starsPerPage = 50; // Show 5 stars per page
    ...
    return (
        <div className={`bg-gray-800 bg-opacity-0 overflow-hidden h-96 relative rounded-lg p-6 shadow-lg mb-5 flex ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
            ...
            <div className='absolute flex justify-center items-center top-10'>
                ...
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '755px ', width: '755px' }}></div>
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '965px ', width: '965px' }}></div>
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '1175px ', width: '1175px' }}></div>
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '1385px ', width: '1385px' }}></div>
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '1595px ', width: '1595px' }}></div>
                <div className='rounded-full border-white border-2  absolute opacity-50 -z-10' style={{ height: '1805px ', width: '1805px' }}></div>
            </div>
            <div className="flex items-center absolute">
                <h2 className="text-3xl font-bold ml-6">{starDetails.hostname}</h2>
            </div>

            <div className="flex ml-40 relative ${index % 2 === 0 ? 'ml-40' : 'mr-40'}`">
                {/* Planet Balls */}
                {planets.map((planet, planetIndex) => (
                    <div key={planetIndex} className='flex justify-center items-center flex-col font-Saira' style={{ marginTop: `${planetIndex * 40}px` }}>
                        <div className="mx-10 w-12 h-12 rounded-full mr-4">
                            <img src={planetImages[Math.floor(Math.random() * planetImages.length)]} alt={planet.pl_name} className="w-full h-full object-cover rounded-full" />
                        </div>
                        {planet.pl_name}
                    </div>
                ))}
