        ...
        const fetchStarsAndPlanets = async () => {
            try {
                ...
                const starData = data.reduce((acc, planet) => {
                    const { hostname } = planet; // Corrected field for star name
                    if (!acc[hostname]) {
                        acc[hostname] = {
                            starDetails: planet,
                            planets: []
...
const StarCard = ({ star }) => {
    ...
    return (
        <div className="bg-gray-800 overflow-hidden relative rounded-lg p-6 shadow-lg mb-5 flex flex-col items-center">
            {/* Star Ball */}
            <div className="relative w-36 h-36 bg-yellow-500 rounded-full flex justify-center items-center mb-4">
                <img src={starImageUrl} alt={starDetails.hostname} className="w-full h-full object-cover rounded-full" />
                <h2 className="absolute text-white text-xl font-bold text-center">{starDetails.hostname}</h2>

                {/* Planets orbiting around the star */}
                {planets.map((planet, index) => (
                    <PlanetOrbit key={index} planet={planet} index={index} />
                ))}
            </div>
        </div>
    );
...
// Component to Position Planets Around the Star
const PlanetOrbit = ({ planet, index }) => {
    const orbitRadius = 70 + index * 30; // Set orbit distance based on planet index
    const angle = index * (360 / 10); // Distribute planets evenly in a circle
    const planetStyle = {
        position: 'absolute',
    ...
    return (
        <div style={planetStyle} className="w-10 h-10 bg-blue-500 rounded-full"></div>
    );
};
