...
const NASA_IMAGE_API_URL = 'https://images-api.nasa.gov/search'; // NASA Image API
...
const ExoplanetPage = () => {
    ...
    // Fetch images for a specific exoplanet using NASA Image API
    const fetchPlanetImage = async (planetName) => {
        try {
            const response = await fetch(`${NASA_IMAGE_API_URL}?q=${planetName}&media_type=image`);
            const data = await response.json();
            if (data.collection.items.length > 0) {
                return data.collection.items[0].links[0].href; // Use the first image found
            }
