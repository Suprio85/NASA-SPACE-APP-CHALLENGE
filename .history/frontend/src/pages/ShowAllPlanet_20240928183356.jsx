...
import PlanetComparison from '../component/planetComparison';
...
const ExoplanetPage = () => {
    ...
    const toggleComparison = () => {
        if (showComparison && planetList.length > 0) {
            if (window.confirm("Are you sure you want to hide the comparison and clear all selected planets?")) {
                setPlanetList([]); // Clear the planetList if user confirms
                setShowComparison(false); // Only set to false directly
            }
        } else {
            setShowComparison(!showComparison); // Toggle normally if no need to clear
        }
    };
    ...
    useEffect(() => {
        ...
        fetchExoplanets();
    }, [planetList, showComparison]);

    const addToPlanetList = (planet) => {
        
        if (!planetList.some(p => p.pl_name === planet.pl_name) && planetList.length < 3) {
            setPlanetList([...planetList, planet]);
        }
    };
    

    const removeFromPlanetList = (planet) => {
        const updatedList = planetList.filter(p => p.pl_name !== planet.pl_name);
    ...
    return (
        <div className="min-h-screen absolute top-0 text-white w-full">
            ...
            {showComparison && <PlanetComparison planetList={planetList} toggle={toggleComparison}/>}
            </div>
            {planetList.length > 0 &&
                <div className=' fixed   w-60  h-screen top-0 left-0 flex justify-start items-center'>
                    <div className=' bg-slate-950 bg-opacity-40 rounded-e-full flex flex-col justify-center items-center' style={{ height: '400px' }}>
                        {
                            planetList.map((planet, index) => (
                                <div key={index} className='flex relative '>
                                    <div className='bg-slate-950 mt-1 ml-2 flex justify-center items-center rounded w-6 mr-1'>
                                        <h1 className='text-white font-Saira font-bold'>{index}</h1>
                                    </div>
                                    <div className='bg-slate-900 mt-1 pl-3 rounded w-44 py-2'>
                                        <h1 className='text-white font-Saira '>{planet.pl_name}</h1>
                                    </div>
                                    <div className='relative right-5 flex items-center' onClick={() => removeFromPlanetList(planet)}>
                                        <button><FontAwesomeIcon icon={faXmark} className='text-white' /></button>
                                    </div>
                                </div>
                            ))
                        }
                        <div className='fixed mt-72 left-5 ' onClick={toggleComparison}>
                            <div className='bg-slate-900 absolute bg-opacity-80 z-10 w-full h-full'></div>
                            
                            <button  className='btn btn-outline overflow-hidden relative' >
                                <div className='z-10 text-slate-300 font-Saira text-xl'  >Compare</div>
