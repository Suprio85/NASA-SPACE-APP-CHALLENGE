...
const SolarSystem = () => {
    ...
    return (
        <div className="flex fixed justify-center items-center h-screen  absolute top-0 left-0 w-full">
                ...
                {planets.map((planet, index) => (
                    <div>
                        ...
                        <div className=' bg-slate-500 bg-opacity-5  rounded-full absolute'
                            style={{
                                width: `${(planet.planetIndex + 1) * 130}px`,
                                height: `${(planet.planetIndex + 1) * 130}px`,
                                top: `calc(50% - ${(planet.planetIndex + 1) * 65}px)`,
                                left: `calc(50% - ${(planet.planetIndex + 1) * 65}px)`,
                            }}>
                        </div>
                    </div>
                ))}
