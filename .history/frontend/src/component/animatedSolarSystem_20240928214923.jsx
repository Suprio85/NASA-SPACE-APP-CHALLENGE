        ...
        <div className="flex justify-center items-center h-screen bg-black">
            <div className="relative w-full h-full flex justify-center items-center">
                ...
                {planets.map((planet, index) => (
                    <div key={index} className={`absolute top-1/2 left-1/2 ${planet.orbitSize} ${planet.speed} flex justify-center items-center`}>
                        {/* Planet positioning: offset from center */}
                        <div className="bg-blue-500 h-8 w-8 rounded-full" style={{ transform: 'translateX(-50%)' }}></div>
                    </div>
                ))}
            </div>
        </div>
