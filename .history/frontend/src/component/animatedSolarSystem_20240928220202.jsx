            ...
            <div className="relative">
                ...
                {planets.map((planet, index) => (
                    <div key={index} className={`absolute top-1/2 left-1/2`} style={{
                        ...
                        transformOrigin: ` ${parseInt(planet.orbitSize)}px` // Different distance from the Sun
                    }}>
                        <div className="bg-blue-500 h-8 w-8 rounded-full absolute -top-4 left-1/2 transform -translate-x-1"></div>
                    </div>
                ))}
            </div>
        </div>
